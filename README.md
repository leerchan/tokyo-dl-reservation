...
- The default polling interval is **30 seconds** (per `ADR-10`). Don't lower it.
  Configurable down to 30 s only; faster polling triggers a hard ADR review.
...
1. Every 30 seconds, queries the Tokyo MPD's 学科試験 reservation calendar
...
> 7. Setting up the launchd agent from `RUNBOOK.md` for 30-second polling.
