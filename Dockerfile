...
CMD ["sh", "-c", "sleep ${START_DELAY:-0}; while :; do dl-poll --config /app/config.local.json --state /app/state/snapshot.json ${DL_RES_FLAGS:---silent-baseline}; sleep ${POLL_INTERVAL:-60}; done"]
